...
1. Order levels by `(elevation, elevationIndex)`. Drop **scaffolding levels** and any level the side-car marks `role: ignore`.
   ...
   **That fallback is gated on parsed rooms, not on an empty resolver result**, and the distinction is load-bearing. The resolver also returns nothing when the model has rooms of which *none* are conditioned — a garage-only model, or every level marked `role: ignore`. That is the resolver answering ("nothing here is conditioned, so nothing has an envelope horizontal"), not failing to; running the bounding-box path over it manufactures a ceiling and a floor for space that carries no supply air. "Identical by construction" holds only when at least one conditioned room resolves.
...
6. The lowest conditioned level's floor face resolves to `ground` (slab), not `void` — **unless that level declares its own `below_void`.** The rule is a default about a house sitting on grade, and `levels.<name>.below_void` is the modeler answering that exact question, so the explicit answer wins. Without the exception a single-storey house with `below_void: outdoor` still emitted a ground-coupled `floor`, which put `exposed_floor` out of reach by its only documented route.

Undrawn space defaults: a void **below** resolves to the `crawlspace` space, a void **above** to `attic` — the overwhelmingly common cases for a house, both overridable per level. The report warns on void area regardless, so the default is never silent. Each gap carries the space it resolved against (`FaceSplit.void_space`), and `Envelope.voids` carries the surface category that became — so a report attributes per void rather than asking the whole envelope which categories it happens to contain, which is a different question with a different answer on any model mixing `below_void`s.
...
assemblies:
  buffer_floor: 0.521            # U-value, a bare scalar — uninsulated floor over the crawl
```
...
**Volume is accounted per LEVEL, not per walled level.** As shipped this walk started as part of the wall walk over `walls_by_level`, which meant a level was only accounted if it carried walls — a conditioned level with rooms and no walls drawn contributed 0 ft³ and quietly took its share of the infiltration term with it. Walls decide surfaces; rooms decide volume; neither gates the other. The scaffolding exclusion is the one thing that must precede *both*: a level excluded from the stack and from volume is not conditioned space, so nothing can be on the thermal envelope between it and outdoors, and its walls must not be classified either.
...
- **Cross-module literal agreement:** `loads.ATTIC_SPACE` and `resolve_faces`'s `above_void` default both spell `"attic"` and are independent literals, and the hot-attic cooling substitution fires only when they match. Rename either and every ceiling silently drops from the sol-air temperature to the bare unvented 0.5 — on Refrhus, a cooling factor of 3.66 becoming 0.50, with no exception raised. Guarded by reading the default off the signature and asserting it equals the constant. The same shape of guard covers `report._VOID_TREATMENT` against `geometry.CATEGORY_FOR_BELOW`: where two modules hold one value by copy because there is no natural constant to share, the copy gets a test.
