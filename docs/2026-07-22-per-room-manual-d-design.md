...
## Scope / honesty
- **Exterior classification is conditioned-aware but level-based.** A wall is on the envelope when a *conditioned* room sits on exactly one side; unconditioned space (garage/crawlspace levels) is excluded. Conditioned interior space not yet drawn as a room still reads as "outdoors" and over-counts until drawn (mirror of the old bounding-box under-count; self-corrects as rooms are added). Because `conditioned` is derived from the level name, a conditioned↔**buffer** partition (a garage on the *same* level as living space) can't be expressed, and a buffer wall doesn't get its reduced (partial-ΔT) treatment — both need per-room conditioned marking. Uses the wall midpoint, so a wall partly interior/partly exterior is classified whole.
