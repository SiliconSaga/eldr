...
- `surfaces` — `[{category, area_ft2, space, assembly}]`, one entry per envelope surface. `space` is `null` for anything not facing a buffer space (every wall, window, door, and an on-grade `floor`); `assembly` is `null` for anything using its category's default U-value, which is every surface in an untagged model.
